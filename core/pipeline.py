...
class PlanningPipeline:
    ...
    def run(self, request: PlanningRequest):
        ...
        # Fetch matching enterprise standards from Snowflake Knowledge Graph
        context_rules = self.storage.get_enterprise_context(request)
        request.enterprise_context = context_rules
        yield {"step": 0, "agent": "Enterprise Context Builder", "status": "complete"}
        ...
        try:
            ...
            metrics = ExecutionMetrics(
                ...
                knowledge_documents_retrieved=len(context_rules),
                knowledge_chunks_retrieved=len(context_rules)
            )
